...
const Milestone = "hyperdensity_production_canary_movement_expansion_from_observation_window_v1"
...
func ValidateSurface(doc map[string]interface{}) error {
	...
	for _, key := range []string{
		...
		"fluidvirtPolicyAuthority", "fluidvirtControllerAuthority",
	...
	if strv(doc["controllerMode"]) == "general_production_auto" || strv(doc["controllerMode"]) == "production_auto_with_policy" {
		return fmt.Errorf("forbidden controllerMode: %s", strv(doc["controllerMode"]))
	}
	if err := validateCanarySections(doc); err != nil {
		return err
	}
	if err := evaluateCanaryExpansionPass(doc); err != nil {
		return err
	}
	if err := evaluateAccounting(doc); err != nil {
		return err
	}
	if len(sliceMap(doc["blockers"])) > 0 && boolv(doc["productionCanaryMovementExecuted"]) {
		return fmt.Errorf("productionCanaryMovementExecuted true while blockers exist")
	}
