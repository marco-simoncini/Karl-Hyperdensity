...
const Milestone = "hyperdensity_controller_runtime_reconciliation_smoke_tick_v1"
...
func ValidateSurface(doc map[string]interface{}) error {
	...
	for _, key := range []string{
		...
		"dashboardExecutor", "fluidvirtPolicyAuthority", "fluidvirtReconcilerAuthority",
	...
	if len(sliceMap(doc["blockers"])) > 0 && boolv(doc["smokeTickPassed"]) {
		return fmt.Errorf("smokeTickPassed true while blockers exist")
	}
