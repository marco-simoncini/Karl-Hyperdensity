#!/usr/bin/env python3
"""Parse examples/khr/**, examples/ingest/**, and examples/grandepadre/** fixtures (stdlib JSON; PyYAML for yaml)."""
...
def main() -> int:
    ...
    if validate_tree(root, Path("examples") / "grandepadre" / "evidence-store", required=True):
        return 1
    if validate_tree(root, Path("examples") / "grandepadre" / "recommendation", required=True):
        return 1
