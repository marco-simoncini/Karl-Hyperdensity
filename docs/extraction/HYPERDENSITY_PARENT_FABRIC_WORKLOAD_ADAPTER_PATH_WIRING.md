...
**Sprint 56** is the **first minimal runtime wiring**: Dashboard routes **approved non-apply** path helper call sites through **`hyperdensityWorkloadPath*V1`** wrappers when **`hyperdensityWorkloadAdapterPathWiredV1 = true`**. **Observed-state** remains legacy. **Karl-Hyperdensity** receives **no** Go adapter code. Full **`workload_helpers.go`** verdict remains **`copy-deferred`**.
