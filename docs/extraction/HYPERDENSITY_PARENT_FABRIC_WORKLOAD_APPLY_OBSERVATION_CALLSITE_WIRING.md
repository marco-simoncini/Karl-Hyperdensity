...
**Sprint 70** is the **first real wiring** on `apply.go`: four legacy observation call sites repointed to **staged wrappers**. **Sprint 71** post-wiring hardening. **Sprint 72** flip criteria. **Sprint 73** candidate-runtime readiness. Flags remain **false** — wrappers delegate to legacy helpers (legacy-equivalent behavior).
